...
total_test = len(test_data)
predict = []
dict_new = {value: key for key, value in intent_dict.items()}
for batch_index, data in enumerate(utils.get_batch(test_data, batch_size=64)):
    ...
    res_test_list = res_test.cpu().numpy()
    predict.extend(replace(res_test_list,dict_new))
